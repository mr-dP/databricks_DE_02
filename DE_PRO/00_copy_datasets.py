...
class CourseDataset:
    ...
    def process_bronze(self):
        ...
        query = (
            ...
            .writeStream.option("checkpointLocation", f"{self.checkpoint}/bronze")
    ...
    def porcess_orders_silver(self):
        ...
        query = (
            deduped_df.writeStream.foreachBatch(self.__upsert_data)
            .outputMode("update")
    ...
    def porcess_customers_silver(self):
        ...
        query = (
            ...
            .writeStream.foreachBatch(self.__batch_upsert)
            .outputMode("update")
    ...
    def porcess_books_silver(self):
        ...
        query = (
            ...
            .select(F.from_json(F.col("value").cast("string"), schema).alias("v"))
            .select("v.*")
            .writeStream.foreachBatch(self.__type2_upsert)
...
dataset_bookstore = "dbfs:/FileStore/demo-datasets/DE-Pro/bookstore"
spark.conf.set(f"dataset.bookstore", dataset_bookstore)
checkpoint_path = "dbfs:/FileStore/demo_pro/checkpoints"
